...
void MeshDeformProcessor::UpdatePointCoordsAndBright_LineMeshLens_Thrust(std::shared_ptr<Particle> p, float* brightness, LineLens3D * l, bool isFreezingFeature, int snappedFeatureId)
{
...
void MeshDeformProcessor::UpdatePointCoordsAndBright_UniformMesh(std::shared_ptr<Particle> particle, float* brightness, float* _mv)
{
...
bool MeshDeformProcessor::ProcessParticleDeformation(float* modelview, float* projection, int winWidth, int winHeight, std::shared_ptr<Particle> particle, float* glyphSizeScale, float* glyphBright, bool isFreezingFeature, int snappedGlyphId, int snappedFeatureId)
{
	if (lenses == NULL || lenses->size() == 0)
		return false;
	Lens *l = lenses->back();
