...
LineSplitModelGrid::LineSplitModelGrid(float dmin[3], float dmax[3], int n, bool useLineSplitGridMesh)
{
	_n = n;
	_dmin[0] = dmin[0];
	_dmin[1] = dmin[1];
...
void LineSplitModelGrid::UpdatePointCoords_LineMeshLens_Thrust(float4* v, int n)
{
	thrust::device_ptr<int> dev_ptr_tet(GetTetDev());
	thrust::device_ptr<float> dev_ptr_X(GetXDev());
	int tet_number = GetTetNumber();
...
void LineSplitModelGrid::UpdatePointCoordsAndBright_LineMeshLens_Thrust(float4* v, float* brightness, int n, LineLens * l, bool isFreezingFeature, int snappedFeatureId, char *feature)
{
	if (isFreezingFeature)
		thrust::copy(&feature[0], &feature[0] + n, d_vec_feature.begin()); //can be placedc at ReinitiateMesh() to improve performance
...
void LineSplitModelGrid::UpdatePointCoords(float4* v, int n, float4* vOri)
{
	int tet_number = GetTetNumber();
	int* tet = GetTet();
	float* X = GetX();
...
//only used for old circle lens object space
void LineSplitModelGrid::InitGridDensity(float4* v, int n)
{
	if (gridType == GRID_TYPE::UNIFORM_GRID)
	{
