...
#ifdef USE_CONTROLLER
	std::shared_ptr<QController> controller;
#endif
...
#ifdef USE_CONTROLLER
	void SlotUpdateControllers(QVector3D leftPos, QVector3D rightPos,
		int numHands, bool pressed);
#endif

	void SlotUpdateLeftHandPos(float x, float y, float z);
};
