...
/*
__global__ void d_render_preint_immerTransp(uint *d_output, uint imageW, uint imageH, float density, float brightness, float3 eyeInLocal, int3 volumeSize, int maxSteps, float tstep, bool useColor, float transparentPenalty, float3 tunnelStart, float3 tunnelEnd, float3 rectVerticalDir, float deformationScale, float deformationScaleVertical, bool *achieved, float lat, float ldt, float lst, float p1t, float p2t, float densityt, float brightnesst)//, char* screenMark)
{
	uint x = blockIdx.x*blockDim.x + threadIdx.x;
	uint y = blockIdx.y*blockDim.y + threadIdx.y;
	...
	bool lastPenaltyTaken = false;

	for (int i = 0; i<maxSteps; i++)
	{
		bool takeTransparencyPenalty = inTunnel2(pos, tunnelStart, tunnelEnd, deformationScale, deformationScaleVertical, rectVerticalDir);
		...
		if (takeTransparencyPenalty){
			//float funcRes = clamp((sample - transFuncP2) / (transFuncP1 - transFuncP2), 0.0, 1.0);

			float funcRes = clamp((sample - p2t) / (p1t - p2t), 0.0, 1.0);
			...
			//if (funcRes > 0)
			//{				//funcRes = clamp(length(normalInWorld), 0.0, 1.0);
			//	funcRes = clamp((length(normalInWorld) - p2t) / (p1t - p2t), 0.0, 1.0);
			//}
			...
			{
				float3 normal_in_eye = normalize(mul(c_NormalMatrix, normalInWorld));
				col = make_float4(phongModelGivenCoeff(cc, posInWorld, normal_in_eye, lat,ldt,lst), funcRes * 1.0);
			}
			else
			{
				col = make_float4(la*cc, funcRes);
		...
		lastPenaltyTaken = takeTransparencyPenalty;
...
__device__ bool
d_posInChannel(float3 pos, int3 dims, float3 spacing)
{
...
__global__ void d_render_preint_immerTransp(uint *d_output, uint imageW, uint imageH, float density, float brightness, float3 eyeInLocal, int3 volumeSize, int maxSteps, float tstep, bool useColor, float transparentPenalty, float3 tunnelStart, float3 tunnelEnd, float3 rectVerticalDir, float deformationScale, float deformationScaleVertical, bool *achieved, float lat, float ldt, float lst, float p1t, float p2t, float densityt, float brightnesst)//, char* screenMark)
{
	...
	bool lastPenaltyTaken = false;
	bool lastSampleSolid = false;
	bool rayEnteredSolid = false;

	float tneart, tfart;
	rectVerticalDir = normalize(rectVerticalDir);
	float3 sideDir = normalize( cross(normalize(tunnelEnd - tunnelStart), rectVerticalDir));
	float3 boxMint = tunnelStart - rectVerticalDir*deformationScaleVertical - sideDir*deformationScale / 2;
	float3 boxMaxt = tunnelEnd + rectVerticalDir*deformationScaleVertical + sideDir*deformationScale / 2;
	//float3 boxMint = tunnelStart - rectVerticalDir*2 - sideDir* 2;
	//float3 boxMaxt = tunnelEnd + rectVerticalDir*2 + sideDir* 2;
	int hitt = intersectBox(eyeRay, boxMint, boxMaxt, &tneart, &tfart);
	//note tneart will always be 0, and it will always hit since transTunnelStart = eyeInLocal. this is only computed for tfart
	...
	for (int i = 0; i<maxSteps; i++)
	{
		...
		bool currentSampleSolid = d_posInChannel(pos, volumeSize, spacing);
		if (currentSampleSolid && !lastSampleSolid && !rayEnteredSolid){
			tneart = t;
			rayEnteredSolid = true;
		}
		if (takeTransparencyPenalty){// && rayEnteredSolid){//rayEnteredSolid && t >= tneart && tneart<tfart){
			//float funcRes = clamp((sample - transFuncP2) / (transFuncP1 - transFuncP2), 0.0, 1.0);

			funcRes = clamp((sample - p2t) / (p1t - p2t), 0.0, 1.0);
			//funcRes /= (brightnesst*4);

			float3 normalInWorld = make_float3(tex3D(volumeTexGradient, coord.x, coord.y, coord.z)) / spacing;

			//if (length(normalInWorld)<brightnesst/10)
			//{
			//	funcRes = 0;
			//}

			//funcRes = clamp(sample * (length(normalInWorld) * 10 * brightnesst - p2t) / (p1t - p2t), 0.0, 1.0);


			//float ttthr = 10;
			//if (t - tneart > ttthr && tfart - t>ttthr)
			//{
			//	funcRes = 0;
			//}
		...
		lastPenaltyTaken = takeTransparencyPenalty;
		lastSampleSolid = currentSampleSolid;
	}
	...
	float3 eyeInLocal, int3 volumeSize, int maxSteps, float tstep, bool useColor, char* screenMark, PositionBasedDeformProcessor *pd, RayCastingParameters *rcpTran)
{
	...
	if (pd->noNeedTransparent()){
		...
		//pd->transparentPenalty = 0.2;
		d_render_preint_immerTransp << <gridSize, blockSize >> >(d_output, imageW, imageH, density, brightness, eyeInLocal, volumeSize, maxSteps, tstep, useColor, pd->transparentPenalty, pd->transTunnelStart, pd->transTunnelEnd, pd->transRectVerticalDir, pd->deformationScale, pd->deformationScaleVertical, pd->d_transparentAchieved, rcpTran->la, rcpTran->ld, rcpTran->ls, rcpTran->transFuncP1, rcpTran->transFuncP2, rcpTran->density, rcpTran->brightness);
