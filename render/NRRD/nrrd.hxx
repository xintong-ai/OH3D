		...
		else
		{
			...
			*data=new T[n];
			...
			if (header_offset <= 0) {//return false;
				std::string datafile = file.substr(0, file.find_last_of("\\/") + 1).append( (*hdr_fields)["data file"]);
				raw.open(datafile.c_str(), std::ios::binary);
			} else {
				raw.open(file.c_str(), std::ios::binary);
